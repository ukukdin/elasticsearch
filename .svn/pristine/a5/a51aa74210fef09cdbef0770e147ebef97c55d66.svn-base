...
<%--
-------------------------------------------------------------------------
Description  :  차단유저대응 > 차단화면 호출조회 - 검색화면
-------------------------------------------------------------------------
--%>
    ...
    <table id="tableForSearch" class="table table-bordered datatable">
        ...
        <tbody>
            <tr>    
                ...
                <th>접속 URL</th>
                <td>
                    <input type="text" name="url" id="url" value="" class="form-control" maxlength="150" />
                </td>
                <td class="noneTd"></td>
...
function executeSearch() {
    ...
    var defaultOptions = {
            url          : "<%=contextPath %>/blockingUser/list_of_blockingPage_results",
